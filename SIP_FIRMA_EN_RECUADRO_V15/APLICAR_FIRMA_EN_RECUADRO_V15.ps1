...
# IMPORTANTE: @() fuerza siempre una colección, aunque haya 0 o 1 resultado.
$backupCandidates = @(
    Get-ChildItem (Split-Path -Parent $pdfService) -File -Filter "pdf_service.py.backup_recuadro_firma_v13_*" -ErrorAction SilentlyContinue |
...
if ($backupCandidates.Count -gt 0) {
    ...
    Write-Host "`n   [AVISO] No encontré backup V13; aplicaré sobre el archivo actual." -ForegroundColor Yellow
}

$stamp = Get-Date -Format "yyyyMMdd_HHmmss"
$backup = "$pdfService.backup_v15_$stamp"
Copy-Item $pdfService $backup -Force
Write-Host "   [OK] Backup V15: $backup" -ForegroundColor Green
...
Write-Host "`n==> Aplicando parche" -ForegroundColor Cyan
& python (Join-Path $PSScriptRoot "patch_recuadro_firma_v15.py") $pdfService
if ($LASTEXITCODE -ne 0) {
    throw "Falló patch_recuadro_firma_v15.py"
}
...
Write-Host " FIRMA EN RECUADRO V15 APLICADA CORRECTAMENTE" -ForegroundColor Green
Write-Host "====================================================" -ForegroundColor Green
Write-Host ""
