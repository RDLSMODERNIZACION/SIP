...
$backupRoot = "$root\_backup_fix_sin_graficos_v5_$stamp"
...
foreach ($pat in $patterns) {
    $r = New-Object Text.RegularExpressions.Regex($pat)
    if ($r.IsMatch($t)) {
...
Write-Host " SIP FIX SIN GRAFICOS V5 APLICADO CORRECTAMENTE" -ForegroundColor Green
Write-Host "====================================================" -ForegroundColor Green
Write-Host ""
