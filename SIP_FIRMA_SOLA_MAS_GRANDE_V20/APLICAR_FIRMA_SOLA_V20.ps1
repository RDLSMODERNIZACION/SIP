...
$backup = "$pdfService.backup_v20_$stamp"
Copy-Item $pdfService $backup -Force
Write-Host "   [OK] Backup: $backup" -ForegroundColor Green

Write-Host "`n==> Copiando firma" -ForegroundColor Cyan
...
Write-Host "`n==> Aplicando parche V20" -ForegroundColor Cyan
& python (Join-Path $PSScriptRoot "patch_v20.py") $pdfService
if ($LASTEXITCODE -ne 0) { throw "Falló patch_v20.py" }
...
Write-Host " FIRMA SOLA V20 APLICADA CORRECTAMENTE" -ForegroundColor Green
Write-Host "====================================================" -ForegroundColor Green
Write-Host "Firma más grande y centrada en ambos recuadros." -ForegroundColor Cyan
