...
const TEMPLATE_HELP: Record<string, string> = {
  ...
  relief_valve_set: "Válvula relief/PRV: requiere apertura/seteo, cierre, hermeticidad, precinto.",
  hydrostatic_line: "Línea/manguera/brida/conexión: requiere parámetros hidrostáticos.",
};
...
function detailToForm(detail: any): CertificateCreatePayload {
  const c = detail?.certificate || {};
  const base = defaultForm(c.certificate_number || "");
  return {
    ...base,
    certificate_number: c.certificate_number || base.certificate_number,
    ...
    document_type: c.document_type || base.document_type,
    template_type: c.template_type || base.template_type,
    md_required: Boolean(c.md_required),
    requires_hydraulic_chart: false,
    responsible_name: c.responsible_name || base.responsible_name,
    responsible_license: c.responsible_license || base.responsible_license,
    asset_unit_code: c.asset_unit_code || "",
    seal_number: c.seal_number || "",
    test_medium: c.test_medium || "",
    ambient_temperature: c.ambient_temperature || base.ambient_temperature,
    client_id: c.client_id || "",
    equipment_id: c.equipment_id || null,
    purchase_order: c.purchase_order || "",
    calibration_date: c.calibration_date || base.calibration_date,
    expiration_date: c.expiration_date || base.expiration_date,
    ...
    test_type: c.test_type || base.test_type,
    reference_method: c.reference_method || base.reference_method,
    environmental_conditions: c.environmental_conditions || base.environmental_conditions,
    measurement_unit: c.measurement_unit || base.measurement_unit,
    observations: c.observations || base.observations,
    conclusions: c.conclusions || base.conclusions,
    trial_result: c.trial_result || base.trial_result,
    approved_result: c.approved_result ?? base.approved_result,
    final_comments: c.final_comments || "",
    is_paid: Boolean(c.is_paid),
    payment_notes: c.payment_notes || null,
    test_rows: (detail?.test_rows?.length ? detail.test_rows : base.test_rows).map((row: any, index: number) => ({
      row_order: Number(row.row_order || index + 1),
      pressure_label: row.pressure_label || `PRESIÓN DE PRUEBA N°${index}`,
      range_value: parseNullableNumber(row.range_value),
      unit: row.unit || c.measurement_unit || base.measurement_unit || "PSI",
      acceptance_criteria: row.acceptance_criteria || "",
      result: row.result || "",
      observations: row.observations || "",
    })),
    pattern_usages: (detail?.patterns || [])
      .map((pattern: any) => pattern.pattern_id)
      .filter(Boolean)
      .map((pattern_id: string) => ({ pattern_id })),
    metrology_results: detail?.metrology_results || base.metrology_results || [],
    sensor_loop_results: detail?.sensor_loop_results || base.sensor_loop_results || [],
    relief_valve_result: detail?.relief_valve_result || base.relief_valve_result || null,
    hydrostatic_result: detail?.hydrostatic_result || base.hydrostatic_result || null,
  };
}
...
export function CertificateFormModal({
  ...
  const modalTitle = isEdit ? "Editar certificado" : "Nuevo certificado";
  ...
  async function loadResources() {
    ...
    const [
      ...
      getEquipment(),
      getPatterns(),
      isEdit ? Promise.resolve({ certificate_number: "" }) : getNextCertificateNumber({ prefix: "SIP" }),
    ...
    if (isEdit && detail) {
      ...
      setForm({
        ...editForm,
        measurement_unit: editForm.measurement_unit || preferredUnit,
        unit: editForm.unit || preferredUnit,
      });
      setSelectedPatternId(String(detail?.patterns?.[0]?.pattern_id || p[0]?.id || ""));
    } else {
      setForm({
        ...baseForm,
        client_id: c[0]?.id || "",
        measurement_unit: preferredUnit,
        unit: preferredUnit,
...
function effectiveRequiresHydraulicChart(templateCode = String(form.template_type || "general_pressure"), manualValue = Boolean(form.requires_hydraulic_chart)) {
  ...
  function updateTestRow(index: number, key: string, value: string) {
    setForm((prev) => {
      ...
      row[key] = key === "range_value" ? parseNullableNumber(value) : value;
      rows[index] = row;
  ...
  async function onSubmit(event: FormEvent) {
    ...
    try {
      ...
      const payload = {
        ...
        test_rows: (form.test_rows || []).map((row, index) => ({
          row_order: Number(row.row_order || index + 1),
          pressure_label: normalizeText(row.pressure_label) || `PRESIÓN DE PRUEBA N°${index + 1}`,
          range_value: parseNullableNumber(row.range_value),
          unit: normalizeText(row.unit) || normalizeText(form.measurement_unit) || "PSI",
          acceptance_criteria: normalizeText(row.acceptance_criteria),
      ...
      <form onSubmit={onSubmit} className="space-y-6">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-4">
          ...
          {(() => {
            ...
            if (info.chartForced) messages.push("");
            if (!info.chartForced && info.chartRequired) messages.push("");
              ...
              <tbody>
                {(form.test_rows || []).map((row, index) => (
                  <tr key={row.row_order} className="border-t border-slate-100">
                    ...
                    <td className="p-2">
                      <div className="flex gap-2">
                        <input className={inputClass} type="text" inputMode="decimal" value={row.range_value ?? ""} onChange={(e) => updateTestRow(index, "range_value", e.target.value)} />
        ...
        <div className="flex flex-col gap-3 border-t border-slate-100 pt-5 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button type="submit" disabled={loading || loadingNumber}>
              {loading ? (isEdit ? "Guardando..." : "Creando...") : (isEdit ? "Guardar cambios" : "Guardar y enviar a aprobación")}
            </Button>
          </div>
        </div>
      </form>
